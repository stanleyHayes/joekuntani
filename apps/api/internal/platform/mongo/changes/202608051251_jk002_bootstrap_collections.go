...
func Registry() []Change {
	...
	}, authSessionsChange(), siteSettingsChange(), mediaAssetsChange(), exactGlobalSettingsChange(), exactMediaAssetsChange(), servicesChange(), contentChange(), serviceLifecycleChange(), contentRevisionChange(), eventsTicketTypesChange(), eventValidationChange(), enquiriesOutboxChange(), ticketOrdersInventoryChange(), enquiryReviewChange(), paymentsWebhooksChange(), ticketIssuanceChange(), crmChange(), crmReviewChange(), ticketDeliveryLeaseChange(), bookingsChange(), campaignsChange(), crmWorkflowChange(), campaignReviewChange(), crmWorkflowReviewChange(), auditExportIndexesChange(), conversionEventsChange(), privacyHoldsChange(), ticketOperationsChange(), checkinChange(), staffAccountSurfacesChange(), supportDonationsChange(), merchChange(), contentSectionsChange(), aboutPageSectionsChange(), videoInfrastructureChange(), videoCategoriesChange(), videoAspectRatioChange(), socialVideoLinksChange()}
}
