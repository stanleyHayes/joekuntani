...
func Registry() []Change {
	...
	}, authSessionsChange(), siteSettingsChange(), mediaAssetsChange(), exactGlobalSettingsChange(), exactMediaAssetsChange(), servicesChange(), contentChange(), serviceLifecycleChange(), contentRevisionChange(), eventsTicketTypesChange(), eventValidationChange(), enquiriesOutboxChange(), ticketOrdersInventoryChange(), enquiryReviewChange(), paymentsWebhooksChange(), ticketIssuanceChange(), crmChange(), crmReviewChange(), ticketDeliveryLeaseChange(), bookingsChange(), campaignsChange(), crmWorkflowChange(), campaignReviewChange(), crmWorkflowReviewChange()}
}
