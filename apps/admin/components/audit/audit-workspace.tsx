...
export function AuditWorkspace() {
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    void load({
      ...
      entityType: String(data.get("entity_type") ?? "").trim(),
    });
  }
    ...
    <section className={styles.workspace} aria-labelledby="audit-heading">
      ...
      <form className={styles.form} role="search" onSubmit={submit}>
        <div className={styles.queryIntro}>
          <span>Refine register</span>
          <h3>Filter recorded actions</h3>
          <p>
      ...
      <p className={styles.status} role="status" aria-live="polite">
        {state === "loading" && "Loading the latest audited actions."}
        {state === "error" ? (
      ...
      {state === "loading" && !response ? (
        <AdminSkeleton label="Loading audit register" variant="table" />
      ) : null}
