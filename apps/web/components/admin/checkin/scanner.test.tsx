...
describe("Scanner", () => {
  it("submits a CSRF-protected scan and shows a privacy-safe result", async () => {
    const fetchMock = vi.fn(async (input: RequestInfo, init?: RequestInit) => {
      ...
      if (url.includes("/count")) {
        return Response.json({ event_id: "018f47f6-9f5d-4d3a-8d4e-45f0f7d4c201", checked_in_count: 0 });
      }
    ...
    expect(await screen.findByRole("heading", { name: "Admitted" })).toBeInTheDocument();
  ...
  it("surfaces offline guidance without calling the scan API", async () => {
    ...
    expect(fetchMock).not.toHaveBeenCalledWith("/api/admin/checkin/scan", expect.anything());
  });
});
