...
describe("AuditWorkspace", () => {
  it("submits filters and renders accessible audit rows", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Free-text filter"), { target: { value: "export" } });
    fireEvent.submit(screen.getByRole("search"));
    expect(await screen.findByText("export.bookings")).toBeInTheDocument();
    expect(fetchMock).toHaveBeenCalledWith(expect.stringContaining("q=export"), expect.objectContaining({ credentials: "same-origin", cache: "no-store" }));
  ...
  it("reports empty and failure states without raw server errors", async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValueOnce({ ok: true, json: async () => ({ query: "", limited: false, items: [] }) })
