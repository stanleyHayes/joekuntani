...
Apply MongoDB changes before provisioning. Generate the encryption key and TOTP secret with an approved secret manager; do not commit or paste them into logs. Supply `APP_ENV`, `MONGODB_URI`, `MONGODB_DATABASE`, `MFA_ENCRYPTION_KEY`, `STAFF_NAME`, `STAFF_EMAIL`, `STAFF_PASSWORD`, `STAFF_ROLE`, and `STAFF_MFA_SECRET` as process environment values, then run:
